...
@app.route("/get-workshops")
def get_workshops_api():
    """
    FIX: Switched to query parameters (?category=...) to handle spaces 
    in 'General Science' reliably on hosted environments.
    """
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        # Using LOWER and TRIM for maximum robustness against DB formatting
        cursor.execute("""
            SELECT id, title 
            FROM workshops 
...
def add_video():
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (data.get('title'), data.get('desc'), data.get('category'), data.get('url')))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
